...
CREATE TABLE IF NOT EXISTS `mydb`.`Partida` (
  ...
  PRIMARY KEY (`id_Partida`, `Campeonato_nome`),
  INDEX `fk_Partida_Campeonato1_idx` (`Campeonato_nome` ASC),
  CONSTRAINT `fk_Partida_Campeonato1`
    FOREIGN KEY (`Campeonato_nome`)
    REFERENCES `mydb`.`Campeonato` (`nomeCampeonato`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `mydb`.`Titulo` (
  ...
  `Campeonato_nomeCampeonato` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`Campeonato_nomeCampeonato`),
  INDEX `fk_Titulo_Campeonato1_idx` (`Campeonato_nomeCampeonato` ASC),
  CONSTRAINT `fk_Titulo_Campeonato1`
    FOREIGN KEY (`Campeonato_nomeCampeonato`)
    REFERENCES `mydb`.`Campeonato` (`nomeCampeonato`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`Partida_has_Time` (
  ...
  `Time_nomeTime_Vencedor` VARCHAR(50) NOT NULL,
  `Time_nomeTime_Perdedor` VARCHAR(50) NULL,
  PRIMARY KEY (`Partida_id_Partida`, `Time_nomeTime_Vencedor`),
